...
(defn create-parsed-jwt
  "Create a JWT token as it is formed when we receive it from Keycloak."
  ([username admin?]
   (create-parsed-jwt username admin?
                      ["beta-tester" "offline_access" "uma_authorization"]))
...
(def token-schnaqqifant-user
  (sjwt/create-signed-jwt (create-parsed-jwt "schnaqqifant" false) config/testing-private-key))
(def token-n2o-admin
  (sjwt/create-signed-jwt (create-parsed-jwt "n2o" true) config/testing-private-key))
(def token-timed-out
  (let [payload (create-parsed-jwt "n2o" true)]
    (sjwt/create-signed-jwt (assoc payload :exp (inc (:iat payload))) config/testing-private-key)))
(def token-wegi-no-beta-user
  (sjwt/create-signed-jwt (create-parsed-jwt "A. Schneider" false ["pleb"]) config/testing-private-key))
